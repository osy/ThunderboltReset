...
// {
        ...
        {
            TBT_ROOT.POC0 = One
            DBG2 ("ICME", TBT_ROOT.ICME)
            If (TBT_ROOT.ICME != 0x800001A6 && TBT_ROOT.ICME != 0x800000A6)
            {
                If (TBT_ROOT.CNHI ())
                {
                    DBG2 ("ICME", TBT_ROOT.ICME)
                    If (TBT_ROOT.ICME != 0xFFFFFFFF)
                    {
                        //SGDI (0x01070004)
                        TBT_ROOT.WTLT ()
                        DBG2 ("ICME", TBT_ROOT.ICME)
                        If (!Local0 = (TBT_ROOT.ICME & 0x80000000)) // NVM started means we need reset
                        {
                            TBT_ROOT.ICME |= 0x06 // invert EN | enable CPU
                            Local0 = 1000
                            While ((Local1 = (TBT_ROOT.ICME & 0x80000000)) == Zero)
                            {
                                Local0--
                                If (Local0 == Zero)
                                {
        ...
        {
            Notify (TBT_ROOT, Zero) // Bus Check
        }
